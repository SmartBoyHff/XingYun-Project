...
public class TutorialManager : MonoBehaviour
{
    ...
    private List<Toggle> stepToggle = new List<Toggle>();
    ...
    private void Awake()
    {
        ...
        //text1.text= GetStepText(steps[0]);
    ...
    private void GenerateStepTexts()
    {
        ...
        for (int i = 0; i < nonEmptyIndices.Count; i++)
        {
            ...
            {
                ToggleStepText(origIdx);
            });
            stepToggle.Add(toggle);
            toggle.enabled = true;
            obj.GetComponentInChildren<UnityEngine.UI.Image>().enabled = true;
            //
    ...
    private void RefreshStepToggle()
    {
        int effectiveCurrent = Mathf.Min(currentStepIndex, steps.Length);

        for (int i = 0; i < stepToggle.Count; i++)
        {
            int origIdx = textOriginalIndices[i];
            int threshold = textCompleteThresholds[i];

            if (effectiveCurrent >= threshold)
                stepToggle[i].isOn = false;
            else if (effectiveCurrent >= origIdx)
                stepToggle[i].isOn = true;

        }
    }

    // --- 按钮与流程控制 ---
    public void OnGenericNextClicked()
    {
        if (!canProceed)
    ...
    private void ExecuteStep(int index)
    {
        ...
        if (!ExamUIManager.Instance.isExam)
    ...
    public void ToggleStepText(int stepInt)
    {
        text1.text = GetStepText(steps[stepInt]);
        text2.text = steps[stepInt].Text;
        text1.color = inProgressTextColor;
